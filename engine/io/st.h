...
/* legge una FETTA di byte grezzi (pesi quantizzati): byte_off relativo all'inizio
 * del tensore. Usato da EMBED_STREAM per una sola riga di embedding. */
static void st_read_raw_slice(shards *S, const char *name, int64_t byte_off, int64_t nbytes, void *out, int drop) {
    st_tensor *t = st_find(S, name);
    if (!t) { fprintf(stderr, "missing tensor: %s\n", name); exit(1); }
    if (byte_off < 0 || nbytes < 0 || byte_off + nbytes > t->nbytes) {
        fprintf(stderr, "raw slice OOB %s off=%lld n=%lld ten=%lld\n",
