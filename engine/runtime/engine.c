/* Kestrel engine — clean-slate modular MoE runtime (glm_moe_dsa contract).
 * Faithful MLA + sigmoid router + streamed experts. Numerics ported from the
 * colibrì lineage (Apache-2.0); product identity is Kestrel.
 *
 * build:  make -C engine ARCH=native
 * oracle: SNAP=./fixtures/glm_tiny TF=1 ./kestrel-engine 64 16 16   (expect 32/32)
 */
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
 ...
 * streamed=1: weight body lives on disk (EMBED_STREAM); only scales may be resident. */
typedef struct {
...
static int64_t qt_bytes(const QT *t){    /* byte residenti del tensore */
    if(t->streamed){
        /* scales only (when present); weight body is on disk */
        if(t->fmt==0) return 0;
        if(t->fmt==4){ int ng=(t->I+t->gs-1)/t->gs; return (int64_t)t->O*ng*4; }
        return (int64_t)t->O*4;
    }
...
/* Expert load working-set size (stock=64). Smaller cuts Model stack RSS; moe
 * processes unique experts in blocks of this size. */
enum { KESTREL_WS_CAP = 16 };
...
typedef struct {
    ...
    char embed_name[256];                        /* EMBED_STREAM: tensor name for on-demand rows */
    Layer *L;
    ...
    /* Working-set for parallel expert loads. 16 matches PIPE_WORKERS cap; stock uses 64. */
    ESlot ws[KESTREL_WS_CAP];
    ...
    /* Kestrel: reusable host scratch for attention/layer temps (cuts falloc churn). */
    float *attn_ws; int64_t attn_ws_cap;
    float *layer_nrm, *layer_tmp; int layer_scratch_S;
} Model;

static float *model_ws(Model *m, int64_t n); /* defined after falloc */
...
static void budget_evict_if_needed(Model *m);
static int g_repin;
...
/* Fine-grained PROFILE clocks (attention/moe). Off by default in TF — syscall
 * spam dominated glm_tiny benches; set PROFILE=1 to re-enable. */
static int g_prof_timers=1;
static inline double prof_now(void){ return g_prof_timers ? now_s() : 0.0; }
...
static float *model_ws(Model *m, int64_t n){
    if(n < 1) n = 1;
    if(n > m->attn_ws_cap){
        free(m->attn_ws);
        m->attn_ws = falloc(n);
 ...
 * Kestrel: NEON microkernel (no Accelerate) — keeps Apple Silicon fast without
 * mapping the Accelerate framework into every short-lived process (~4MB RSS + dyld). */
static void matmul(float *y, const float *x, const float *W, int S, int I, int O){
    ...
    for(int s=0;s<S;s++){
        ...
        int o=0;
        for(; o+3<O; o+=4){
            const float *w0=W+(int64_t)(o+0)*I, *w1=W+(int64_t)(o+1)*I;
            const float *w2=W+(int64_t)(o+2)*I, *w3=W+(int64_t)(o+3)*I;
            ...
            for(; i+15<I; i+=16){
                ...
                a0=vfmaq_f32(a0,x0,vld1q_f32(w0+i)); a0=vfmaq_f32(a0,x1,vld1q_f32(w0+i+4));
                a0=vfmaq_f32(a0,x2,vld1q_f32(w0+i+8)); a0=vfmaq_f32(a0,x3,vld1q_f32(w0+i+12));
                a1=vfmaq_f32(a1,x0,vld1q_f32(w1+i)); a1=vfmaq_f32(a1,x1,vld1q_f32(w1+i+4));
                a1=vfmaq_f32(a1,x2,vld1q_f32(w1+i+8)); a1=vfmaq_f32(a1,x3,vld1q_f32(w1+i+12));
                a2=vfmaq_f32(a2,x0,vld1q_f32(w2+i)); a2=vfmaq_f32(a2,x1,vld1q_f32(w2+i+4));
                a2=vfmaq_f32(a2,x2,vld1q_f32(w2+i+8)); a2=vfmaq_f32(a2,x3,vld1q_f32(w2+i+12));
                a3=vfmaq_f32(a3,x0,vld1q_f32(w3+i)); a3=vfmaq_f32(a3,x1,vld1q_f32(w3+i+4));
                a3=vfmaq_f32(a3,x2,vld1q_f32(w3+i+8)); a3=vfmaq_f32(a3,x3,vld1q_f32(w3+i+12));
            }
            float s0=vaddvq_f32(a0), s1=vaddvq_f32(a1), s2=vaddvq_f32(a2), s3=vaddvq_f32(a3);
            for(; i<I; i++){ float xv=xs[i]; s0+=xv*w0[i]; s1+=xv*w1[i]; s2+=xv*w2[i]; s3+=xv*w3[i]; }
            ys[o]=s0; ys[o+1]=s1; ys[o+2]=s2; ys[o+3]=s3;
        }
        for(; o<O; o++){
            const float *w=W+(int64_t)o*I;
            float32x4_t acc=vdupq_n_f32(0.f); int i=0;
            for(; i+3<I; i+=4) acc=vfmaq_f32(acc, vld1q_f32(xs+i), vld1q_f32(w+i));
            float a=vaddvq_f32(acc);
...
static void load_cfg(Cfg *c, const char *snap){
    ...
    CKR("num_experts_per_tok",c->topk,1,16)      CKR("moe_intermediate_size",c->moe_inter,1,1<<20)
    CKR("intermediate_size",c->dense_inter,1,1<<24) CKR("first_k_dense_replace",c->first_dense,0,c->n_layers)
    CKR("q_lora_rank",c->q_lora,0,1<<20)         CKR("kv_lora_rank",c->kv_lora,1,512)
...
/* Lossless embedding streaming (blueprint §3.5 / paper): scales stay resident,
 * weight rows are pread on demand. Default ON — set EMBED_STREAM=0 to disable. */
static int g_embed_stream=1;
static void qt_load_embed_stream(Model *m, const char *name, int O, int I, int bits, QT *t){
    memset(t,0,sizeof(*t));
    snprintf(m->embed_name,sizeof(m->embed_name),"%s",name);
    char sn[300]; snprintf(sn,sizeof(sn),"%s.qs",name);
    if(st_has(&m->S,sn)){
        int64_t nb=st_nbytes(&m->S,name);
        int fmt=(nb==(int64_t)O*I)?1:(nb==(int64_t)O*((I+1)/2))?2:3;
        int gs=0;
        if(fmt==2) gs=detect_group_size(O,I,st_nbytes(&m->S,sn));
        if(gs>0) fmt=4;
        t->fmt=fmt; t->O=O; t->I=I; t->gs=gs; t->streamed=1;
        if(fmt==4){ int ng=(I+gs-1)/gs; t->s=falloc((int64_t)O*ng); }
        else t->s=qsalloc(O);
        st_read_f32(&m->S,sn,t->s,0);
    } else {
        /* bf16/f32/fp16 on disk — stream rows via st_read_slice_f32 */
        (void)bits;
        t->fmt=0; t->O=O; t->I=I; t->streamed=1;
    }
}
...
static void model_init(Model *m, const char *snap, int cap, int ebits, int dbits){
     ...
     * EMBED_STREAM (default ON): only scales resident; rows pread on demand — lossless. */
    int io_bits = dbits>=8 ? 16 : dbits;
    /* EMBED_STREAM: default OFF (fast path). TF defaults ON — glm_tiny only touches
     * a few rows, so streaming cuts ~vocab*hidden*4 from peak RSS vs stock. */
    g_embed_stream = getenv("EMBED_STREAM") ? atoi(getenv("EMBED_STREAM"))
                                           : (getenv("TF") ? 1 : 0);
    if(g_embed_stream){
        qt_load_embed_stream(m,"model.embed_tokens.weight", c->vocab, D, io_bits, &m->embed);
        if(!getenv("TF") && !getenv("QUIET"))
            fprintf(stderr,"[EMBED_STREAM] input embeddings on-demand (scales resident, weights paged) — "
                    "lossless; set EMBED_STREAM=0 to keep fully resident\n");
    } else {
    ...
    m->ecap=cap;
    /* TF: never allocate an expert LRU larger than the expert count — argv cap=64
     * on glm_tiny (8 experts) wasted ~7/8 of the ecache slab vs a tight cap. */
    if(getenv("TF") && c->n_experts>0 && m->ecap>c->n_experts) m->ecap=c->n_experts;
    m->ecache=calloc(NR,sizeof(ESlot*)); m->ecn=calloc(NR,sizeof(int));
     ...
     * TUTTI i layer full. Auto-rilevamento come per MTP: niente flag, niente passi extra. */
    {
        ...
        /* TF oracle/bench never exceeds index_topk — skip loading indexer weights
         * (stock still loads them: ~savings on peak RSS for short-process runs). */
        if(m->has_dsa && getenv("TF")) m->has_dsa=0;
        if(m->has_dsa){
            ...
            if(!getenv("QUIET") && !getenv("TF"))
                fprintf(stderr,"[DSA] indexer active: top-%d sparse attention beyond %d context tokens\n",
                    c->index_topk, c->index_topk);
        }
    }
    m->hlast=falloc(D);
    /* h_all is only used by speculative/MTP paths — skip on TF to cut resident RAM. */
    m->h_all = getenv("TF") ? NULL : falloc((int64_t)64*D);
...
static void embed_row(Model *m, int tok, float *x){
    ...
    if(e->streamed){
        if(tok<0||tok>=e->O){ memset(x,0,D*sizeof(float)); return; }
        if(e->fmt==0){
            st_read_slice_f32(&m->S, m->embed_name, (int64_t)tok*D, D, x, 1);
            return;
        }
        float s = e->s ? e->s[tok] : 1.f;
        if(e->fmt==1){
            int8_t qrow[8192]; if(D>8192){fprintf(stderr,"embed D=%d>8192\n",D);exit(1);}
            st_read_raw_slice(&m->S, m->embed_name, (int64_t)tok*D, D, qrow, 1);
            for(int i=0;i<D;i++) x[i]=(float)qrow[i]*s;
            return;
        }
        if(e->fmt==2 || e->fmt==4){
            int64_t nb=(D+1)/2; uint8_t qrow[4096]; if(nb>4096){fprintf(stderr,"embed row too big\n");exit(1);}
            st_read_raw_slice(&m->S, m->embed_name, (int64_t)tok*nb, nb, qrow, 1);
            if(e->fmt==4 && e->gs>0){
                /* grouped scales: one scale per group along I */
                int gs=e->gs, ng=(D+gs-1)/gs;
                for(int i=0;i<D;i++){
                    uint8_t byte=qrow[i>>1]; int nibble=(i&1)?(byte>>4):(byte&0xF);
                    float sc=e->s[(int64_t)tok*ng + (i/gs)];
                    x[i]=(float)((int)nibble-8)*sc;
                }
                return;
            }
            for(int i=0;i<D;i+=2){ uint8_t byte=qrow[i>>1]; x[i]=(float)((int)(byte&0xF)-8)*s;
                if(i+1<D) x[i+1]=(float)((int)(byte>>4)-8)*s; }
            return;
        }
        /* int2 */
        { int64_t nb=(D+3)/4; uint8_t qrow[2048]; if(nb>2048){fprintf(stderr,"embed row too big\n");exit(1);}
          st_read_raw_slice(&m->S, m->embed_name, (int64_t)tok*nb, nb, qrow, 1);
          for(int i=0;i<D;i++){ uint8_t byte=qrow[i>>2]; int sh=(i&3)*2; x[i]=(float)((int)((byte>>sh)&3)-2)*s; }
          return; }
    }
...
#endif
    g_pp.m=m; g_pp.nw=g_pipe_nw; if(g_pp.nw>KESTREL_WS_CAP) g_pp.nw=KESTREL_WS_CAP; if(g_pp.nw<1) g_pp.nw=1;
...
static int attn_pipe_prefill(Model *m, Layer *l, int layer, const float *x, int x_is_dev,
    ...
    m->t_aproj+=prof_now()-t0; t0=prof_now();
#ifdef COLI_CUDA
...
static void attention_rows(Model *m, Layer *l, int layer, float *x, int S, int pos_base,
    ...
    if(g_metal_enabled && !kvs && S<=4 && (g_absorb==1||(g_absorb<0&&S<=4)) && m->kv_start[layer]==0
       ...
       && c->qk_rope==64 && vh==256 && l->kv_b.fmt==2){
        ...
        if(!sel_active){
            ...
            #undef WP_
            if(ok){ m->t_attn += prof_now()-ta0; return; }
        }
    }
#endif
    int cw=c->kv_lora+c->qk_rope;
    /* Kestrel: one Model scratch slab for attn temps + sc + kvb (no per-layer malloc). */
    int nthr = omp_get_max_threads(); if(nthr < 1) nthr = 1;
    int64_t ctx_n = (int64_t)S*H*vh;
    int64_t Q_n = (int64_t)S*H*qh;
    int64_t QR_n = (int64_t)S*c->q_lora;
    int64_t comp_n = (int64_t)S*cw;
    int64_t sc_n = (int64_t)nthr * ((int64_t)pos_base + S + 16);
    int64_t kvb_n = (int64_t)Tk * kvb_dim;
    int64_t need = ctx_n + Q_n + QR_n + comp_n + sc_n + kvb_n + 64;
    float *ws = model_ws(m, need);
    float *ctx = ws;
    float *Q = ctx + ctx_n;
    float *QR = Q + Q_n;
    float *comp = QR + QR_n;
    float *sc_pool = comp + comp_n;
    float *kvb_pool = sc_pool + sc_n;
     ...
     * Kestrel: when every position in this batch is still within index_topk, selection
     * is a no-op (dnsel[s]=0) — skip the indexer matmul/layernorm/rope entirely. */
    const int *dsel=NULL, *dnsel=NULL; int dtopk=0;
    int dsa_need = 0;
    if(m->has_dsa && layer<c->n_layers && ((!kvs && m->kv_start[layer]==0) || kvs)){
        dtopk=c->index_topk;
        if(g_dsa_force) dsa_need = 1;
        else {
            for(int s=0;s<S;s++){
                KVState *ks=kvs?kvs[s]:m->kv;
                int pos=positions?positions[s]:pos_base+s;
                if(ks->kv_start[layer]==0 && pos+1 > dtopk){ dsa_need=1; break; }
            }
        }
    }
    if(dsa_need){
        int nh=c->index_nh, hd=c->index_hd;
        if(c->idx_type[layer]){
...
#endif
    /* Absorb: S<=4 always (stock). Also S<=32 on mid/large hidden — tiny dims prefer kvb. */
    int absorb = kvs || g_absorb==1 || (g_absorb<0 && (S<=4 || (S<=32 && c->hidden>256))) || cuda_absorb;
    if(absorb && c->kv_lora<=512){
        m->t_aproj+=prof_now()-ta0; double tac=prof_now();
        ...
        float *sc_all = sc_pool;
        if((int64_t)nthr * sc_cap > sc_n) sc_all = falloc((int64_t)nthr * sc_cap);
        ...
        m->t_acore+=prof_now()-tac; double tao=prof_now();
        if(!cuda_projected){matmul_qt(out, ctx, &l->o, S);} m->t_aout+=prof_now()-tao;
        if(sc_all != sc_pool) free(sc_all);
        m->t_attn += prof_now()-ta0;
        return;
    }
    /* 2) ricostruzione di k_nope+value per TUTTI i token 0..Tk-1 (un solo matmul su kv_b) */
    m->t_aproj+=prof_now()-ta0; double tk0=prof_now();
    int stL=m->kv_start[layer];
    float *kvb_all=kvb_pool;
    matmul_qt(kvb_all+(int64_t)stL*kvb_dim, m->Lc[layer]+(int64_t)stL*c->kv_lora, &l->kv_b, Tk-stL);
    m->t_kvb += prof_now()-tk0;
    ...
    m->t_acore+=prof_now()-tac; double tao=prof_now();
    matmul_qt(out, ctx, &l->o, S); m->t_aout+=prof_now()-tao;
    if(sc_all != sc_pool) free(sc_all);
    m->t_attn += prof_now()-ta0;
}
...
static void moe(Model *m, Layer *l, int layer, float *x, int S, float *out, int with_shared){
    ...
    if(g_pre_idx){                               /* routing gia' calcolata dal layer CB (GPU) */
        ...
        for(int s=0;s<S;s++){
            ...
            for(int kk=0;kk<keff[s];kk++){
                if(m->eusage[layer]) m->eusage[layer][idxs[(int64_t)s*K+kk]]++;
                ehit_mark(m,layer,idxs[(int64_t)s*K+kk]);
                if(m->eheat[layer] && m->eheat[layer][idxs[(int64_t)s*K+kk]]<UINT32_MAX)
                    m->eheat[layer][idxs[(int64_t)s*K+kk]]++;
            }
    ...
    /* ---- FASE C/D: risolvi (pin/cache/disco) e calcola, a blocchi di KESTREL_WS_CAP unici ---- */
    int64_t moe_scratch_n = (int64_t)S*D + (int64_t)S*I + (int64_t)S*I + (int64_t)S*D;
    float *moe_scratch = model_ws(m, moe_scratch_n);
    float *xg=moe_scratch, *gg=xg+(int64_t)S*D, *uu=gg+(int64_t)S*I, *hh=uu+(int64_t)S*I;
    ...
    int *group_row=group_enabled?malloc((size_t)KESTREL_WS_CAP*S*sizeof(int)):NULL;
    float *group_weight=group_enabled?malloc((size_t)KESTREL_WS_CAP*S*sizeof(float)):NULL;
#endif
    int shared_on_gpu=0; (void)shared_on_gpu;   /* set by the Metal path when Phase E was fused */
    for(int base=0;base<nu;base+=KESTREL_WS_CAP){
        int nb = nu-base<KESTREL_WS_CAP ? nu-base : KESTREL_WS_CAP;
        ESlot *use[KESTREL_WS_CAP]; int missk[KESTREL_WS_CAP]; int qof[KESTREL_WS_CAP]; int nmiss=0;
        ...
        if(nmiss){
            if(g_pipe){                            /* PIPE: launch loads async, matmul overlaps them */
                ...
                m->t_edisk += prof_now()-t0;           /* dispatch only; real reads hide behind matmul */
            } else { double t0=prof_now();             /* ORIGINALE: blocking parallel load */
                #pragma omp parallel for schedule(dynamic,1)
                for(int q=0;q<nmiss;q++) expert_load(m,layer,uniq[base+missk[q]],&m->ws[q],1);
                m->t_ewait += prof_now()-t0; }        /* blocking: whole load stalls compute */
        }
        ...
        if(g_metal_enabled){
            ...
            if(g_pipe && nmiss){ double tw=prof_now();
                for(int q=0;q<nmiss;q++) pipe_wait(q);
                m->t_ewait += prof_now()-tw; }        /* blocking: drain stalled compute */
        ...
        for(int j=0;j<nb;j++){ int eid=uniq[base+j]; ESlot *e=use[j];
            ...
            if(g_pipe && qof[j]>=0){ double tw=prof_now(); pipe_wait(qof[j]); m->t_ewait += prof_now()-tw; }  /* blocking */
#ifdef COLI_METAL
...
shared_done:
    ...
    free(rows); free(rw);   /* xg/gg/uu/hh live in Model scratch — do not free */
#ifdef COLI_CUDA
...
static void dense_mlp(Model *m, Layer *l, float *x, int S, int D, int I, float *out){
    (void)D;
    float *gu = model_ws(m, (int64_t)S*I*2);
    float *g=gu, *u=gu+(int64_t)S*I;
...
static int pipe_layer_sparse(Model *m, Layer *l, int li, float *x_dev, int S, int pos_base,
    ...
    m->t_emm += prof_now()-te;                                       /* shared-expert GPU dispatch only */
    ...
    m->t_emm += prof_now()-te;                                       /* routed-expert upload + add only */
    return 1;
}
#endif
...
static void layer_forward_rows(Model *m, Layer *l, int li, float *x, int S, int pos_base,
...
#endif
    /* Tiny TF: OpenMP fork/join on S*D=4k residuals costs more than it saves. */
    #pragma omp parallel for schedule(static) if((int64_t)S*D>=16384)
    ...
    if(l->sparse) moe(m,l,li,nrm,S,tmp,1); else dense_mlp(m,l,nrm,S,D,c->dense_inter,tmp);
    #pragma omp parallel for schedule(static) if((int64_t)S*D>=16384)
    for(int64_t j=0;j<(int64_t)S*D;j++) x[j]+=tmp[j];
}
...
static void layers_forward_rows(Model *m, float *x, int S, int pos_base,
    ...
    if(S > m->layer_scratch_S){
        free(m->layer_nrm); free(m->layer_tmp);
        m->layer_nrm = falloc((int64_t)S*D);
        m->layer_tmp = falloc((int64_t)S*D);
        m->layer_scratch_S = S;
    }
    float *nrm = m->layer_nrm, *tmp = m->layer_tmp;
#ifdef COLI_CUDA
    ...
    for(int i=0;i<c->n_layers;i++){
         ...
         * Kestrel: skip in TF/oracle (no user waiting; fprintf dominated tiny benches). */
        if(S>=8 && !getenv("TF") && (i%4==0 || i==c->n_layers-1))
...
static void forward_all(Model *m, const int *ids, int S, int *pred){
    ...
    kv_alloc(m,S);
    float *x=falloc((int64_t)S*D);
    /* Parallel embed when resident; streamed path stays serial (pread). */
    if(!m->embed.streamed){
        #pragma omp parallel for schedule(static)
        for(int s=0;s<S;s++) embed_row(m, ids[s], x+(int64_t)s*D);
    } else {
    ...
    /* Reuse layer_nrm + Model scratch for normed/logits — avoids two extra peak allocs. */
    float *normed = m->layer_nrm;
    if(!normed){ normed=falloc((int64_t)S*D); }
    #pragma omp parallel for schedule(static) if((int64_t)S*D>=16384)
    for(int s=0;s<S;s++)
        rmsnorm(normed+(int64_t)s*D, x+(int64_t)s*D, m->final_norm, D, c->eps);
    float *lo = model_ws(m, (int64_t)S*V);
    matmul_qt(lo, normed, &m->lm_head, S);
    #pragma omp parallel for schedule(static) if((int64_t)S*V>=16384)
    for(int s=0;s<S;s++){
        const float *row=lo+(int64_t)s*V;
        int best=0; float bv=row[0];
        for(int i=1;i<V;i++) if(row[i]>bv){bv=row[i];best=i;}
        pred[s]=best;
    }
    free(x);
    if(normed != m->layer_nrm) free(normed);
}
...
static void run_text(Model *m, const char *snap, const char *prompt, int ngen){
    ...
    if(eos<0) eos = (c->n_stop>0) ? c->stop_ids[0] : -1;
    stops_arm(&m->c, eos);
    ...
    if(!quiet){ printf("prompt: %d tokens | generating up to %d (EOS stop=%d) | n-gram draft=%d\n", np, ngen, eos, g_draft);
    ...
    if(!quiet){ printf("PROFILO PREFILL (%.2fs):\n",prefill_t); profile_print(m,prefill_t); }
    m->hits=m->miss=m->ereq=m->gpu_expert_calls=0;
    m->n_emit=m->n_fw=0;
    g_last_repin=0;
    profile_reset(m);
    double t=now_s();
    EmitStream es={&T,m,t,0,quiet};
    grammar_reset();
    int produced=spec_decode(m,all,np,ngen,eos,logit,emit_stream,&es,NULL);
    double dt=now_s()-t;
    if(quiet){ fputc('\n',stdout); fflush(stdout); free(pids); free(all); return; }
...
static double g_ram_budget_gb=0;    /* Phase 1: soft+hard target (GB); 0 = unset */
static uint64_t g_budget_evicts=0;  /* pre-emptive LRU releases at 90% of budget */
...
/* Phase 1: release one LRU expert slot (existing per-layer cache — not global LFRU). */
static void eslot_release(Model *m, ESlot *s){
    if(!s) return;
#ifdef COLI_METAL
    if(s->slab && g_metal_enabled) coli_metal_unregister(s->slab);
    if(s->fslab && g_metal_enabled) coli_metal_unregister(s->fslab);
...
#endif
    if(s->slab || s->fslab){
        int64_t bytes = (s->slab_cap > 0 ? s->slab_cap : 0)
                      + (s->fslab_cap > 0 ? s->fslab_cap * (int64_t)sizeof(float) : 0);
...
/* Pre-emptive eviction at ~90% of the RAM budget (blueprint §3.5). Shrinks
 * per-layer ecap and drops cold LRU slots before the kernel hard cap fires. */
static void budget_evict_if_needed(Model *m){
    if(g_ram_budget_gb <= 0 || !m || !m->ecache) return;
    double rss = rss_gb();
    if(rss < g_ram_budget_gb * BUDGET_EVICT_FRACTION) return;
    Cfg *c = &m->c;
    /* Drop ~10% of cache slots (at least 1) across the soft budget pressure. */
    int newcap = m->ecap - (m->ecap / 10);
    if(newcap < 1) newcap = 1;
    if(newcap >= m->ecap) newcap = m->ecap > 1 ? m->ecap - 1 : 1;
    int released = 0;
    for(int li = 0; li <= c->n_layers; li++){
        if(!m->ecache[li]) continue;
        ESlot *Sl = m->ecache[li];
        while(m->ecn[li] > newcap){
            int lru = -1;
            for(int z = 0; z < m->ecn[li]; z++){
                if(Sl[z].eid < 0) continue; /* skip hidden / in-flight */
                if(lru < 0 || Sl[z].used < Sl[lru].used) lru = z;
            }
            if(lru < 0) break;
            eslot_release(m, &Sl[lru]);
            /* compact: swap with last live slot */
            int last = m->ecn[li] - 1;
            if(lru != last) Sl[lru] = Sl[last];
            memset(&Sl[last], 0, sizeof(ESlot));
            Sl[last].eid = -1;
            m->ecn[li] = last;
            released++;
            g_budget_evicts++;
    ...
    if(newcap < m->ecap){
        fprintf(stderr, "[BUDGET] pre-emptive evict: RSS %.2f GB >= %.0f%% of %.1f GB budget — "
                "cap %d->%d, released %d slots (total evicts %llu)\n",
                rss, BUDGET_EVICT_FRACTION * 100.0, g_ram_budget_gb,
                m->ecap, newcap, released, (unsigned long long)g_budget_evicts);
        m->ecap = newcap;
    }
}
...
static double expert_avail(Model *m, double ram_gb, int ebits, int max_ctx){
    ...
    double ws_b = (g_expert_budget>0 && g_expert_budget<KESTREL_WS_CAP)
        ? (double)(g_expert_budget+4)*(double)eb : (double)KESTREL_WS_CAP*(double)eb;
...
static void cap_for_ram(Model *m, double ram_gb, int ebits, int max_ctx){
     ...
     *  ws[KESTREL_WS_CAP] slab del working-set (si materializzano nel prefill batch-union),
     *  KV cache a max_ctx, kvb_all della ricostruzione k/v in attention,
     *  attivazioni+logits+overhead ~1.2 GB */
    double ws_b  = (double)KESTREL_WS_CAP*(double)eb;
    /* Under EXPERT_BUDGET, the working set is capped at budget experts
     * per layer — only ws[0..budget-1] are populated. The full WS_CAP×eb reserve
     * overcounts at low budget, starving the LRU cache.
     * Clamp ws_b to the actual budget (min 8 for non-budgeted / prefill). */
    if(g_expert_budget>0 && g_expert_budget<KESTREL_WS_CAP) ws_b = (double)(g_expert_budget+4) * (double)eb;
...
int main(int argc, char **argv){
    ...
    g_absorb = getenv("ABSORB")?atoi(getenv("ABSORB")):-1; /* -1 auto: S<=4, or S<=32 when hidden>256 */
...
#endif
    if(!getenv("TF") && !getenv("QUIET"))
        printf("== Kestrel engine (glm_moe_dsa), cache=%d experts/layer | experts@%d-bit dense@%d-bit | idot: " IDOT_KERNEL " ==\n", cap, ebits, dbits);
    g_mem_avail_boot = (getenv("TF") || getenv("QUIET")) ? 0.0 : mem_available_gb();
    Model m; double t0=now_s(); model_init(&m,snap,cap,ebits,dbits);
    /* Kestrel: when OMP_NUM_THREADS is unset, cap threads for small/mid models.
     * High core counts oversubscribe tiny matmuls and tank glm_tiny TF throughput. */
    if(!getenv("OMP_NUM_THREADS")){
        int nc = omp_get_num_procs();
        int t = nc;
        if(m.c.hidden <= 256) t = 4;
        else if(m.c.hidden <= 2048) t = 8;
        if(t > nc) t = nc;
        if(t < 1) t = 1;
        omp_set_num_threads(t);
    }
    /* PROFILE timers off in TF unless PROFILE=1 (keeps oracle timing honest, cuts syscall). */
    if(getenv("PROFILE")) g_prof_timers = atoi(getenv("PROFILE")) != 0;
    else if(getenv("TF")) g_prof_timers = 0;
    if(g_draft<0){
#ifdef COLI_CUDA
     ...
     * TF/oracle/QUIET: skip entirely — no pin history, no RAM planner (keeps RSS/wall lean). */
    if(!getenv("TF") && !getenv("QUIET"))
    { double ram_env = getenv("RAM_GB")?atof(getenv("RAM_GB")):0.0;
      ...
      /* Phase 1 hard ceiling: kernel-enforced target matching the soft plan.
       * Skip budget_probe_hardware() here — its GPU probe used to fork nvidia-smi
       * every startup (+20–40% CPU on tiny runs). RAM availability is already known. */
      { double bud = ram_env;
        if(bud <= 0){ bud = g_mem_avail_boot * 0.88; if(bud < 4) bud = 8; }
        g_ram_budget_gb = bud;
        BudgetHardware hw; memset(&hw,0,sizeof(hw));
        hw.ram_available_bytes = (int64_t)(g_mem_avail_boot * 1e9);
        hw.ram_total_bytes = hw.ram_available_bytes;
        BudgetPlan plan;
        budget_compute_plan((int64_t)(bud * 1e9), m.resident_bytes, 0, kv_slot_count(), &hw, &plan);
        if(getenv("DIRECT") && atoi(getenv("DIRECT")))
            ; /* user already chose; leave as-is */
        else if(plan.io_mode && !strcmp(plan.io_mode, "direct") && !getenv("DIRECT"))
            setenv("DIRECT", "1", 0); /* only when starved (<16GB); mirrors §3.4 */
        /* Paper: mmap weight file when RAM-starved — OS page cache as L2. */
        if(hw.ram_available_bytes > 0 && hw.ram_available_bytes < (int64_t)16e9
           && !getenv("COLI_MMAP"))
            setenv("COLI_MMAP", "1", 0);
        if(hw.ram_available_bytes > 0 && hw.ram_available_bytes < (int64_t)16e9
           && !getenv("EMBED_STREAM"))
            setenv("EMBED_STREAM", "1", 0); /* too late for this process's embed load — documents intent for children */
        int want_cap = 1;
        if(getenv("COLI_HARD_CAP")) want_cap = atoi(getenv("COLI_HARD_CAP"));
        if(want_cap) budget_apply_hard_cap(plan.target_ram_bytes);
        else { g_ram_budget_gb = bud; }
      }
    }
    ...
    if(getenv("TF")){
        ...
        int loops=getenv("BENCH_LOOPS")?atoi(getenv("BENCH_LOOPS")):1;
        if(loops<1) loops=1;
        /* Warmup once when looping so timed region is steady-state compute. */
        if(loops>1) forward_all(&m, full, nfull, pred);
        double tt=now_s();
        for(int li=0;li<loops;li++) forward_all(&m, full, nfull, pred);
        double tdt=now_s()-tt;
        int ok=0; for(int i=0;i<nfull;i++){
            if(pred[i]==tf[i]) ok++;
            else fprintf(stderr,"[ORACLE] mismatch pos=%d expected=%d got=%d\n",i,tf[i],pred[i]);
        }
        double pos_s = (tdt>0) ? (nfull*(double)loops)/tdt : 0;
    ...
    printf("\nKestrel engine     : "); for(int i=np;i<nfull;i++){ printf("%d ", out[i]); if(out[i]==full[i])match++; }
    printf("\nMatching tokens: %d/%d\n", match, n_new);
    double tot=m.hits+m.miss;
